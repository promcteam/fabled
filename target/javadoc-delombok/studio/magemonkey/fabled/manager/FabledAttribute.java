...
public class FabledAttribute implements IconHolder {
    ...
    public ItemStack getIcon(PlayerData data) {
        ...
        if (meta != null && iconMeta != null) {
            ...
            List<String> lore = iconLore != null ? iconLore.stream().map(iconLine -> filter(data, iconLine)).collect(Collectors.toList()) : new ArrayList<>();
    ...
    private String filter(PlayerData data, String text) {
        return  // iomatix: it's a stage now.
        // iomatix: the limit, to verify: is it conflicting with skills' {max} ?
        // iomatix: it's an old {amount}
        text.replace("{amount}", "" + data.getInvestedAttributeStage(key)).replace("{max}", String.valueOf(this.getMax())).replace("{total}", "" + data.getAttribute(key)).replace("{cost}", "" + data.getAttributeUpCost(key)).replace("{invested}", "" + data.getInvestedAttribute(key)).replace("{ap}", "" + data.getAttributePoints()); // iomatix: current value of last attr points
    }
    ...
    public ItemStack getToolIcon() {
        ...
        List<String> lore = iconMeta.hasLore() ? iconMeta.getLore() : null;
        if (lore == null) {
            lore = new ArrayList<>();
        }
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public void setCostModifier(final double costModifier) {
