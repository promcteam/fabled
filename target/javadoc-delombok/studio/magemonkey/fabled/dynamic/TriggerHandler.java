...
public class TriggerHandler implements Listener {
    ...
    public TriggerHandler(final DynamicSkill skill, final String key, final Trigger<?> trigger, final TriggerComponent component) {
    ...
    public void register(final Fabled plugin) {
        plugin.getServer().getPluginManager().registerEvent(trigger.getEvent(), this, EventPriority.HIGHEST, getExecutor(trigger), plugin, !trigger.getEvent().getTypeName().equals("org.bukkit.event.player.PlayerInteractEvent") && !trigger.getEvent().getTypeName().contains("PlayerSwapHandItemsEvent"));
    }

    <T extends Event> void apply(final T event, final Trigger<T> trigger) {
        final LivingEntity caster = trigger.getCaster(event);
        if ((caster instanceof Player && ((Player) caster).getGameMode() == GameMode.SPECTATOR) || caster == null || !active.containsKey(caster.getEntityId())) {
            return;
        }
